...
$(document).ready(function(){
  ...
  $('form')
    .bind('ajax:success', function(event, data, xhr) {
      ...
      var append = $(event.currentTarget).data('append');
    ...
    $('.submit').bind('ajax:complete', function(){
       window.location.href = "/admin/members"
    });
    $("input[name = 'commit']").click(function(){
        if ($('.error').is(':visible')) {
            $('.add').css("margin-top", "-25px");
    ...
    $('input[type=file]').on('change', function() {
        ...
        {
            alert("Allowed file size exceeded. (Max. 5 MB)" +"\n"+ "Your File Size Is " + this.files[0].size/(1024*1024).toFixed(1) + "MB")
            this.value = null;
        }
    });
});
